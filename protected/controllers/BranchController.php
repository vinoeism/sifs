...
class BranchController extends Controller
{
	...
	{
                $addressDataProvider= new CActiveDataProvider('Address',
                            array(
                                'criteria'=>array(
                                    'condition'=>'branch_id=:branchId',
                                    'params'=>array(':branchId'=>  $this->loadModel($id)->id),
                                ),
                                'pagination'=>array(
                                    'pageSize'=>1,
                                )
                ...
                $bankDataProvider= new CActiveDataProvider('Bank',
                            array(
                                'criteria'=>array(
                                    'condition'=>'branch_id=:branchId',
                                    'params'=>array(':branchId'=>  $this->loadModel($id)->id),
...
//		));
	...
	public function actionAdmin()
	{
		...
		$model->unsetAttributes();  // clear any default values
                ...
                {
                    $accessibleBranches[] = $branch->id;
                }               

		if(isset($_GET['Branch']))
			$model->attributes=$_GET['Branch'];
                $model->id = $accessibleBranches;
